...
class MolotovBarApiCacheInterceptor extends DioCacheInterceptor {
  MolotovBarApiCacheInterceptor({required CacheOptions options})
  ...
  void onResponse(
    ...
    if (response.requestOptions.path.endsWith('/cocktails')) {
      var cocktails = response.data['cocktails'] as List<dynamic>;
      if (cocktails == []) {
        return;
      }
      for (var element in cocktails) {
        var cocktailJson = element as Map<String, dynamic>;
        final modifiedResponse = Response(
          requestOptions: response.requestOptions.copyWith(
            path: response.requestOptions.path + '/' + cocktailJson['id'].toString(),
          ),
          data: cocktailJson,
